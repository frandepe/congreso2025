...
export const Comite = ({ testimonials }: { testimonials: Testimonial[] }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl text-center">
      <div className="relative h-32">
        <div className="pointer-events-none absolute left-1/2 top-0 h-[480px] w-[480px] -translate-x-1/2 before:absolute before:inset-0 before:-z-10 before:rounded-full before:bg-gradient-to-b before:from-green-500/25 before:via-yellow-500/5 before:via-25% before:to-fuchsia-500/0 before:to-75%">
        ...
        {testimonials.map((testimonial, index) => (
          <button
            ...
            className={`m-1.5 inline-flex justify-center whitespace-nowrap rounded-full px-3 py-1.5 text-xs shadow-sm transition-colors duration-150 focus-visible:outline-none focus-visible:ring focus-visible:ring-fuchsia-300 dark:focus-visible:ring-fuchsia-600 ${
              active === index
                ? "bg-primary text-white shadow-fuchsia-950/10"
                : "bg-white text-primary hover:bg-fuchsia-100"
            }`}
            onClick={() => {
              setActive(index);
              setAutorotate(false);
          ...
          >
            <span
              className={`${
                active === index ? "text-secondary" : "text-secondary/80"
              }`}
            >
              -
            </span>{" "}
